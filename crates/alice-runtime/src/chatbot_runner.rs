...
pub async fn run_chatbot(
    ...
    let shared: Vec<Arc<tokio::sync::Mutex<Box<dyn ChatAdapter>>>> =
...
async fn handle_event(
    ...
    let (thread_id, text) = match event {
    ...
    let session_id = format!("{adapter_name}-{thread_id}");

    match handle_input_with_skills(&ctx, &session_id, &text).await {
        Ok(AgentLoopOutput::Quit) => {
            tracing::info!("quit signal received from session {session_id}");
        }
        Ok(ref output) => {
